...
一个本地优先的北大树洞关注归档与迁移用户脚本。本仓库只负责官网内的数据导出、关注迁移和归档交付，不扩展为桌面知识库。

## 当前功能

- 从当前登录账号导出全部关注、指定收藏分组、指定 PID 或日期范围；日期按浏览器本地自然日计算并包含首尾。
...
- 单次关注尝试的 POST 不自动重试；用户手动重试未完整项时，会先读取当前状态，确认仍未关注才发起一次新的 POST，并再次核对最终状态。
...
Toolkit 保持为轻量用户脚本：后续只围绕官网 API/DOM 兼容、导出完整性、安全迁移和归档交付维护。桌面数据库、搜索、标签、笔记和 AI 不在本仓库开发。[`方案设计.md`](./方案设计.md) 是已归档的早期方案，不再作为开发路线图。
